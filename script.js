...
// --- Render ---
function renderResults(pkg, ctx) {
  const platformLabel = ctx.platformLabel;

  document.getElementById('content-positioning').innerHTML =
    `<p>${pkg.positioning}</p>`;

  document.getElementById('content-pillars').innerHTML =
    `<ol>
      <li><strong>${pkg.pillars[0]}</strong> &mdash; content that educates and builds trust around ${ctx.product}.</li>
      <li><strong>${pkg.pillars[1]}</strong> &mdash; content that connects emotionally with ${ctx.audience}.</li>
      <li><strong>${pkg.pillars[2]}</strong> &mdash; content that drives action and showcases ${ctx.offer}.</li>
    </ol>`;

  document.getElementById('content-posts').innerHTML =
    `<ol>
      ${pkg.posts.map((p, i) => `<li><strong>Post ${i + 1}:</strong> ${p}</li>`).join('')}
    </ol>`;

  document.getElementById('content-captions').innerHTML =
    pkg.captions.map((c, i) =>
      `<p><strong>Caption ${i + 1}:</strong><br>${c}</p>`
    ).join('');

  document.getElementById('content-ads').innerHTML =
    `<p><span class="badge">${platformLabel}</span></p>` +
    pkg.ads.map((a, i) =>
      `<p><strong>Ad Option ${i + 1}:</strong><br>${a}</p>`
    ).join('');

  // Add platform context to creative
  document.getElementById('content-creative').innerHTML =
    `<p><span class="badge">${platformLabel}</span></p>
     <p><strong>Headline:</strong><br>${pkg.creative}</p>
     <p style="margin-top:8px;font-size:0.85rem;color:var(--color-text-secondary);">
       Suggested visual: high-quality image of {product} with bold overlay text.
       For ${platformLabel}, use 1:1 square format.
     </p>`
    .replace(/\{product\}/g, ctx.product);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const ctx = {
      ...
      product: document.getElementById('productService').value.trim(),
      audience: document.getElementById('targetAudience').value.trim(),
      offer: document.getElementById('mainOffer').value.trim(),
      platform: document.getElementById('platform').value,
      platformLabel: document.getElementById('platform').value === 'facebook' ? 'Facebook' : 'Instagram',
      tone: document.getElementById('tone').value,
      language: document.getElementById('language').value
    };

    const pkg = generate(ctx);
    renderResults(pkg, ctx);
